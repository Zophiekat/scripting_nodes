...
class SN_SetPropertyNode(SN_ScriptingBaseNode, bpy.types.Node):
    ...
    def evaluate(self, context):
        from_sock = self.inputs[1].from_socket() if self.inputs[1].is_linked else None
        if self.inputs[1].is_linked and from_sock:
            if getattr(from_sock, "setattr_source", ""):
                source = from_sock.setattr_source
                attr   = from_sock.setattr_attr
                self.code = f"""
                            if {attr} is not None:
                                setattr({source}, {attr}, {self.inputs[2].python_value})
