...
class SN_SnippetVarsPropertyGroup(bpy.types.PropertyGroup):
    ...
    use_custom: bpy.props.BoolProperty(
        ...
        name="Use Custom",
        description="Use custom variable instead of the standalone snippet variable",
        default=False,
    )
...
class SN_SnippetNode(SN_ScriptingBaseNode, bpy.types.Node):
    ...
    def variable_evaluate(self, snippet):
        for var_tree in snippet["variable_defs"]:
            self.code_imperative += (
                var_tree.replace("SNIPPET_VARS", f"vars_{self.static_uid}") + " = {"
            )
            for var in snippet["variable_defs"][var_tree]:
                for col_var in self.var_collection:
                    if var == col_var.python_name:
                        if not col_var.use_custom or (
                            col_var.use_custom
                            and (
                                not col_var.ref_ntree
                                or not col_var.var_name in col_var.ref_ntree.variables
                                or col_var.type
                                != col_var.ref_ntree.variables[
                                    col_var.var_name
                                ].variable_type
                            )
                        ):
                            self.code_imperative += (
                                f"'{var}': {snippet['variable_defs'][var_tree][var]}, "
                            )
            self.code_imperative += "}\n"
        ...
        for prop in snippet["properties_defs"]:
            for col_prop in self.prop_collection:
                if prop == col_prop.python_name:
                    if not col_prop.use_custom or (
                        col_prop.use_custom
                        and (
                            not col_prop.prop_name in bpy.context.scene.sn.properties
                            or col_prop.type
                            != bpy.context.scene.sn.properties[
                                col_prop.prop_name
                            ].property_type
                            or col_prop.attach_to
                            != bpy.context.scene.sn.properties[
                                col_prop.prop_name
                            ].attach_to
                        )
                        ...
                        self.code_register += (
                            snippet["properties_defs"][prop][0].replace(
                                "SNIPPET_VARS", f"vars_{self.static_uid}"
                            )
                            + "\n"
                        )
                        self.code_unregister += (
                            snippet["properties_defs"][prop][1].replace(
                                "SNIPPET_VARS", f"vars_{self.static_uid}"
                            )
                            + "\n"
                        )
    ...
    def custom_var_props_evaluate(self, code):
        for var in self.var_collection:
            if (
                var.use_custom
                and (bool(var.ref_ntree) and bool(var.var_name))
                and (var.type == var.ref_ntree.variables[var.var_name].variable_type)
            ):
                code = code.replace(
                    var.tree_name
                    + f"_vars_{self.static_uid}['"
                    + var.python_name
                    + "']",
                    var.ref_ntree.variables[var.var_name].data_path,
                )
    ...
    def evaluate(self, context):
        if self.data:
            ...
            self.code_register += snippet["register"].replace(
                "SNIPPET_VARS", f"vars_{self.static_uid}"
            )
            self.code_unregister += snippet["unregister"].replace(
                "SNIPPET_VARS", f"vars_{self.static_uid}"
            )

            code = "\n" + normalize_code(snippet["function"])
            code = code.replace(
                snippet["func_name"], snippet["func_name"] + "_" + self.static_uid
            )
            code = code + "\n" + snippet["imperative"] + "\n"
            ...
            index = (
                1
                if len(self.inputs)
                and self.inputs[0].bl_idname
                in ["SN_InterfaceSocket", "SN_ExecuteSocket"]
                else 0
            )
                ...
                if len(self.inputs) and self.inputs[0].bl_idname == "SN_ExecuteSocket":
                    ...
                    for i, out in enumerate(self.outputs[1:]):
                        return_values.append(
                            get_python_name(
                                f"{out.name}_{i}_{self.static_uid}",
                                f"parameter_{i}_{self.static_uid}",
                            )
                        )
                    ...
                    if len(self.outputs) > 1:
                        ...
                        self.outputs[-1].python_value = (
                            f"{snippet['func_name']}_{self.static_uid}({inp_values})"
                        )
    ...
    def draw_node(self, context, layout):
        if self.wrong_version:
            ...
            row.alert = True
            row.label(
                text="You need to select a snippet that was created using Serpens 3!"
            )
        if not self.data:
        ...
        for var in self.var_collection:
            if var.customizable:
                ...
                if var.use_custom:
                    ...
                    row.operator("sn.tooltip", text="", icon="QUESTION").text = (
                        "Choose a '"
                        + var.type
                        + "' variable to assign this snippet variable to."
                    )
                    if var.ref_ntree and var.var_name in var.ref_ntree.variables:
                        if (
                            var.type
                            != var.ref_ntree.variables[var.var_name].variable_type
                        ):
                            row = layout.row()
                            row.alert = True
                ...
                if prop.use_custom:
                    ...
                    if scene_prop:
                        if (
                            ...
                            row.alert = True
                            row.label(
                                text=f"Choose a property of type '{prop.type}', that's attached to '{prop.attach_to}'!"
                            )
