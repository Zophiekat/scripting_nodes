...
class SN_PanelNode(SN_ScriptingBaseNode, bpy.types.Node):
    ...
    shortcut_only: bpy.props.BoolProperty(
        default=False,
        name="Shortcut Only",
    ...
    is_subpanel: bpy.props.BoolProperty(
        default=False,
        name="Is Subpanel",
    ...
    panel_parent: bpy.props.StringProperty(
    ...
    idname_override: bpy.props.StringProperty(
        default="",
    ...
    last_idname: bpy.props.StringProperty(
        name="Last Idname",
    ...
    def evaluate(self, context):
        ...
        if (
            self.is_subpanel
            and parent
            and not context.scene.sn.is_exporting
            and self.parent_type == "CUSTOM"
        ):
    ...
    def draw_node(self, context, layout):
        if not self.shortcut_only:
            if not self.is_subpanel:
                ...
                row.scale_y = 1.4
                op = row.operator(
                    "sn.activate_panel_picker",
                    text=f"{self.space.replace('_', ' ').title()} {self.region.replace('_', ' ').title()} {self.context.replace('_', ' ').title()}",
                    icon="EYEDROPPER",
                )
                ...
                if self.parent_type == "BLENDER":
                    op = row.operator(
                        "sn.activate_subpanel_picker",
                        text=f"{self.panel_parent.replace('_PT_', ' ').replace('_', ' ').title()}",
                        icon="EYEDROPPER",
                    )
                    op.node_tree = self.node_tree.name
                    op.node = self.name
                    ...
                    if (
                        self.ref_SN_PanelNode == self.name
                        and self.ref_ntree == self.node_tree
                    ):
                        layout.label(
                            text="Can't use self as panel parent!", icon="ERROR"
                        )
