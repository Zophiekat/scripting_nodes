...
class SN_RunInterfaceFunctionNodeNew(SN_ScriptingBaseNode, bpy.types.Node):
    ...
    def on_ref_update(self, node, data=None):
        if node.bl_idname == "SN_InterfaceFunctionNode" and data:
            ...
            # input has changed
                ...
                self.convert_socket(
                    self.inputs[data["changed"].index - index],
                    data["changed"].bl_idname,
                )
            # input has updated
            elif "updated" in data:
                if data["updated"].index - index < len(self.inputs):
                    self.inputs[data["updated"].index - index].name = data[
                        "updated"
                    ].name
    ...
    def update_function_reference(self, context):
        ...
        if self.ref_SN_InterfaceFunctionNode in parent_tree.nodes:
            for out in parent_tree.nodes[self.ref_SN_InterfaceFunctionNode].outputs[
                1:-1
            ]:
                if out.bl_idname != "SN_InterfaceSocket":
    ...
    ref_SN_InterfaceFunctionNode: bpy.props.StringProperty(
        name="Function",
    ...
    def draw_node(self, context, layout):
        ...
        subrow.enabled = (
            self.ref_ntree != None
            and self.ref_SN_InterfaceFunctionNode in self.ref_ntree.nodes
        )
        op = subrow.operator(
