...
class SN_DisplaySearchNode(SN_ScriptingBaseNode, bpy.types.Node):
    ...
    def evaluate(self, context):
        if len(self.inputs["String/Pointer"].links) and len(
            ...
            self.code = f"""{self.active_layout}.prop_search({self.inputs['String/Pointer'].python_source}, '{self.inputs['String/Pointer'].python_attr.replace("'",'"')}', {self.inputs['Collection'].python_source}, '{self.inputs['Collection'].python_attr.replace("'",'"')}', text={self.inputs['Label'].python_value}, icon={self.inputs['Blender Icon'].python_value}, item_search_property="name")"""
        else:
