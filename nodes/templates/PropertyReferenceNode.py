...
class PropertyReferenceNode:
        ...
        if self.prop_name and prop_src:
            ...
            if prop:
                ...
                if "Value" in self.outputs:
                    ...
                    self.convert_socket(
                        self.outputs["Value"], self.socket_names[socket_name]
                    )
    ...
    def prop_source_items(self, context):
        items = [
            ("ADDON", "Addon", "Addon Properties"),
            ("NODE", "Node", "Node Properties"),
        ]
        return items

    prop_source: bpy.props.EnumProperty(
    ...
    from_prop_group: bpy.props.BoolProperty(
    ...
    ref_ntree: bpy.props.PointerProperty(
        ...
        name="Node Tree",
        description="Node Tree to select the property node from",
    ...
    def get_prop_source(self):
        ...
        if self.prop_source == "ADDON":
            ...
            if (
                self.ref_ntree
                and self.from_node
                and self.from_node in self.ref_ntree.nodes
            ):
    ...
    def draw_reference_selection(
        self, layout, unique_selection=False, draw_prop_source=True
    ):
        if draw_prop_source:
            ...
            if self.ref_ntree:
                ...
                if self.from_node in self.ref_ntree.nodes:
                    if not hasattr(self.ref_ntree.nodes[self.from_node], "properties"):
                        self.draw_warning(
                            layout, "The selected node has no properties!"
                        )
        ...
        if unique_selection:
            if self.prop_name:
                for node in self.collection.nodes:  # TODO for NODE
                    if (
                        node != self
                        and self.prop_source == node.prop_source
                        and self.prop_name == node.prop_name
                    ):
                        if self.prop_source == "ADDON" or (
                            self.prop_source == "NODE"
                            and self.from_node == node.from_node
                            and self.ref_ntree == self.ref_ntree
                        ):
                            if (
                                self.from_prop_group
                                and node.from_prop_group
                                and self.prop_group == node.prop_group
                            ):
                                self.draw_warning(
                                    layout, "Multiple nodes found for this property!"
                                )
                                break
