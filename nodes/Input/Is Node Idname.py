...
class SN_NodeIsIdname(SN_ScriptingBaseNode, bpy.types.Node):
    ...
    def on_create(self, context):
        ...
        for name in dir(bpy.types):
            ...
            if (
                hasattr(cls, "bl_rna")
                and cls.bl_rna.base
                and "Node" in cls.bl_rna.base.bl_rna.name
            ):
                item = self.nodes.add()
    ...
    def evaluate(self, context):
        if self.node:
            self.outputs[0].python_value = (
                f"{self.inputs[0].python_value}.bl_rna.identifier == '{self.nodes[self.node].identifier}'"
            )
        else:
    ...
    def draw_node(self, context, layout):
        layout.prop_search(
            self, "node", self, "nodes", text="", item_search_property="name"
        )
