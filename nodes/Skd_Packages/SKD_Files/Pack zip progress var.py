...
class SN_SKD_PackAndMoveNodeThreadProgressVariable(SN_ScriptingBaseNode, bpy.types.Node, VariableReferenceNode):
    bl_idname = "SN_SKD_PackAndMoveNodeThreadProgressVariable"
    bl_label = "Pack And Move Zip File Progress Variable"
    ...
    def evaluate(self, context):
        ...
        self.code_imperative = f"""
                                ...
                                def pack_zip_var(source_path, destination_path, zip_file_name, completion_callback):
                                    {var.data_path} = 0.0
                                    ...
                                    def update_progress():
                                        ...
                                        with thread.lock:
                                            ...
                                            if not thread.completed:
                                                {var.data_path} = round(thread.progress, 2)
                                                return 0.1
                                            {var.data_path} = 100
                                            return None
        ...
        self.code = f"""zip_loc_threaderz{self.static_uid} = pack_zip_var({self.inputs[1].python_value}, {self.inputs[2].python_value}, {self.inputs[3].python_value}, completion_callback)"""
        self.outputs[1].python_value = f"zip_loc_threaderz{self.static_uid}"
