...
def load_images():
    ...
    for file in os.listdir(
        os.path.join(os.path.dirname(os.path.dirname(__file__)), "assets", "tutorial")
    ):
        if file.split(".")[-1] == "jpg":
            images.append(file)
...
class SN_TutorialNode(SN_ScriptingBaseNode, bpy.types.Node):
    ...
    show_settings: bpy.props.BoolProperty(
        default=False, name="Show Settings", description="Show the settings"
    )
    ...
    def update_shader(self, context):
        ...
        dir_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "assets", "tutorial"
        )
        img = bpy.data.images.load(
            os.path.join(dir_path, images[self.index]), check_existing=True
        )
    ...
    def draw_node(self, context, layout):
        ...
        if self.index == ADVENTURE_INDEX:
            ...
            col.scale_y = 2
            op = col.operator(
                "sn.move_tutorial", text="Keep Going!", icon="OUTLINER_DATA_LIGHT"
            )
            op.node = self.name
            op.forward = True
            col.operator(
                "sn.end_tutorial", text="ADVENTURE!!!", icon="OUTLINER_DATA_ARMATURE"
            ).node = self.name
            ...
            op.forward = True
            row.operator(
                "sn.end_tutorial", text="", icon="HANDLETYPE_VECTOR_VEC"
            ).node = self.name
        ...
        if self.index == CHAPTER_INDEX:
            ...
            for index, chapter in enumerate(self.chapters):
                op = col.operator(
                    "sn.set_tutorial", text=str(index + 1) + " | " + chapter[0]
                )
                op.node = self.name
                for i, image in enumerate(images):
                    if image == chapter[1]:
