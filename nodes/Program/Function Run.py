...
class SN_RunFunctionNode(SN_ScriptingBaseNode, bpy.types.Node):
    ...
    ref_SN_FunctionReturnNode: bpy.props.StringProperty(
        name="Return",
            ...
            if self.require_execute:
                ...
                if len(self.outputs) > 2:
                    ...
                    self.outputs[-1].python_value = (
                        f"{parent_tree.nodes[self.ref_SN_FunctionNode].func_name}({inp_values})"
                    )
        else:
            for out in self.outputs[1:]:
                out.reset_value()
    ...
    def draw_node(self, context, layout):
        ...
        subrow.enabled = (
            self.ref_ntree != None
            and self.ref_SN_FunctionReturnNode in self.ref_ntree.nodes
        )
        op = subrow.operator(
            "sn.find_node", text="", icon="RESTRICT_SELECT_OFF", emboss=False
        )
        ...
        if (
            ...
            if (
                ...
                row.alert = True
                row.label(
                    text="Return node not connected to function node!", icon="ERROR"
                )
=======
import bpy
from ...utils import get_python_name
...
class SN_RunFunctionNode(SN_ScriptingBaseNode, bpy.types.Node):
    ...
    def evaluate(self, context):
        ...
        if self.ref_SN_FunctionNode in parent_tree.nodes:
            ...
            for i, inp in enumerate(self.inputs[1:]):
                if inp.bl_idname == "SN_PropertySocket":
                    if inp.python_source != inp.python_value and not inp.python_is_attribute:
                        # Meaningful owner.attr split (e.g. "bpy.data.objects['Cube'].name").
                        # Pass as two args so the function body can use getattr/setattr.
                        inp_values.append(inp.python_source)
                        inp_values.append(f"'{inp.python_attr}'")
                    else:
                        # Plain variable reference or bracket item access ([key]).
                        # No attr to split off.
                        inp_values.append(inp.python_value)
    ...
    def draw_node(self, context, layout):
        ...
        subrow_button = subrow.row()
        subrow_button.enabled = (
            self.ref_ntree != None and self.ref_SN_FunctionNode in self.ref_ntree.nodes
        )
        op = subrow_button.operator(
            "sn.find_node", text="", icon="RESTRICT_SELECT_OFF", emboss=False
        )
        op.node_tree = self.ref_ntree.name if self.ref_ntree else ""
        op.node = self.ref_SN_FunctionNode
        ...
        subrow_return = row.row()
        subrow_return.enabled = (
            self.ref_ntree != None
            and self.ref_SN_FunctionReturnNode in self.ref_ntree.nodes
        )
        op = subrow_return.operator(
            "sn.find_node", text="", icon="RESTRICT_SELECT_OFF", emboss=False
        )
        op.node_tree = self.ref_ntree.name if self.ref_ntree else ""
        op.node = self.ref_SN_FunctionReturnNode
