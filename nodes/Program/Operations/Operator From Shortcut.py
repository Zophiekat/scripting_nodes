    ...
    def execute(self, context):
        if self.button:
            bpy.ops.node.add_node(
                "INVOKE_DEFAULT", type="SN_ButtonNodeNew", use_transform=True
            )
        else:
            bpy.ops.node.add_node(
                "INVOKE_DEFAULT", type="SN_RunOperatorNode", use_transform=True
            )
...
class SN_FindShortcutOperator(SN_ScriptingBaseNode, bpy.types.Node):
    ...
    def draw_node(self, context, layout):
        ...
        layout.prop_search(
            self, "selected", self, "options", text="", item_search_property="name"
        )
        row = layout.row(align=True)
        ...
        op.operator = (
            ""
            if not self.selected in self.options
            else self.options[self.selected].operator
        )
        op.settings = (
            ""
            if not self.selected in self.options
            else self.options[self.selected].settings
        )
    ...
    def find_operators_for_key(self):
        ...
        for keymap in bpy.context.window_manager.keyconfigs.user.keymaps:
            for item in keymap.keymap_items:
                if (
                    item.type == self.key
                    and item.shift == self.shift
                    and item.ctrl == self.ctrl
                    and item.alt == self.alt
                    and item.oskey == self.oskey
                ):
                    if item.idname:
                        props = ""
                        prop_str = ""
                        ...
                        name = (
                            item.name
                            if item.name
                            else item.idname.replace("_", " ").title()
                        )
                        area = (
                            keymap.space_type.replace("_", " ").title()
