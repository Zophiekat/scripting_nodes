...
class SN_FunctionNode(SN_ScriptingBaseNode, bpy.types.Node):
    ...
    def evaluate(self, context):
        out_values = []        # flat param list for the function signature
        out_socket_values = [] # one entry per socket for setting python_value

        for i, out in enumerate(self.outputs[1:-1]):
            param_name = get_python_name(out.name, f"parameter_{i}", lower=False)
            if out.bl_idname == "SN_PropertySocket":
                # Two params: owner object + attr name.
                # Since user can order sockets arbitrarily, we can't use =None 
                # (default args must be at the end). Function Run node passes None explicitly.
                out_values.append(param_name)
                out_values.append(f"{param_name}_attr")
                # Guard: if no attr was supplied the param IS the resolved value.
                out_socket_values.append(
                    f"(getattr({param_name}, {param_name}_attr) if {param_name}_attr else {param_name})"
                )
            else:
                out_values.append(param_name)
                out_socket_values.append(param_name)
        ...
        for i, out in enumerate(self.outputs[1:-1]):
            out.python_value = out_socket_values[i]
            if out.bl_idname == "SN_PropertySocket":
                param_name = get_python_name(out.name, f"parameter_{i}", lower=False)
                out.setattr_source = param_name
                out.setattr_attr   = f"{param_name}_attr"
            else:
                if hasattr(out, "setattr_source"):
                    out.setattr_source = ""
                    out.setattr_attr   = ""
