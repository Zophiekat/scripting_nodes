...
class SN_OnKeypressNode(SN_ScriptingBaseNode, bpy.types.Node):
    ...
    key: bpy.props.StringProperty(
        name="Key", default="Y", update=SN_ScriptingBaseNode._evaluate
    )
    ...
    value: bpy.props.EnumProperty(
        ...
        items=[
            ("PRESS", "On Press", "The action is run when the key is pressed once"),
            ("RELEASE", "On Release", "The action is run when the key is released"),
            (
                "CLICK",
                "On Click",
                "The action is run when the mouse button is clicked once",
            ),
            (
                "DOUBLE_CLICK",
                "On Double Click",
                "The action is run when the mouse button is clicked twice",
            ),
            ("ANY", "Any", "The action will run with any of the above ways"),
            (
                "CLICK_DRAG",
                "On Click Drag",
                "The action is run when you click and drag",
            ),
        ],
        update=SN_ScriptingBaseNode._evaluate,
    )
    ...
    direction: bpy.props.EnumProperty(
        ...
        items=[
            (
                "ANY",
                "Any Direction",
                "The action will run when you click and drag in any direction",
            ),
            (
                "NORTH",
                "North",
                "The action will run when you click and drag in this direction",
            ),
            (
                "NORTH_EAST",
                "North East",
    ...
    def get_spaces(self, context):
        ...
        for item in list(space_names.keys())[1:]:
            items.append(
                (item, item.replace("_", " ").title(), item.replace("_", " ").title())
            )
        return items
    ...
    ref_SN_PieMenuNode: bpy.props.StringProperty(
        name="Menu",
        description="The menu to open with this shortcut",
        update=SN_ScriptingBaseNode._evaluate,
    )
    ...
    def on_ref_update(self, node, data=None):
        if node.bl_idname in ["SN_PanelNode", "SN_MenuNode", "SN_PieMenuNode"]:
            ...
            on_operator_ref_update(
                self, node, data, self.ref_ntree, self.ref_SN_OperatorNode, 0
            )
        ...
        if self.key:
            ...
            if self.action == "PANEL":
                if self.parent_type == "BLENDER" and self.panel:
                    ...
                    and self.ref_ntree
                    and self.ref_SN_PanelNode in self.ref_ntree.nodes
                ):
                    operator = "wm.call_panel"
                ...
                if self.parent_type == "BLENDER" and self.menu:
                    ...
                    self.parent_type == "CUSTOM"
                    and self.ref_ntree
                    and self.ref_SN_MenuNode in self.ref_ntree.nodes
                ):
                    operator = "wm.call_menu"
                ...
                if self.parent_type == "BLENDER" and self.pie:
                    ...
                    self.parent_type == "CUSTOM"
                    and self.ref_ntree
                    and self.ref_SN_PieMenuNode in self.ref_ntree.nodes
                ):
                    operator = "wm.call_menu_pie"
    ...
    def draw_node(self, context, layout):
        ...
        layout.operator("sn.record_key", text=self.key, depress=self.recording).node = (
            self.name
        )
        row = layout.row(align=True)
        ...
        row = layout.row(align=True)
        row.prop(self, "name")
        row.operator("sn.find_referencing_nodes", text="", icon="VIEWZOOM").node = (
            self.name
        )
        layout.separator()
        ...
        if self.action == "PANEL":
            if self.parent_type == "BLENDER":
                ...
                subrow.prop_search(
                    self,
                    "ref_SN_PanelNode",
                    parent_tree.node_collection("SN_PanelNode"),
                    "refs",
                    text="",
                    icon="VIEWZOOM",
                    item_search_property="name",
            ...
            if self.parent_type == "BLENDER":
                name = (
                    self.menu.replace("_", " ").title() if self.menu else "Select Menu"
                )
                op = layout.operator("sn.pick_interface", text=name, icon="EYEDROPPER")
                op.node = self.name
                op.selection = "MENUS"
            ...
            if self.parent_type == "BLENDER":
                name = (
                    self.pie.replace("_", " ").title()
                    if self.pie
                    else "Select Pie Menu"
                )
                op = layout.operator("sn.pick_interface", text=name, icon="EYEDROPPER")
                op.node = self.name
                op.selection = "MENUS"
                ...
                subrow.prop_search(
                    self,
                    "ref_SN_PieMenuNode",
                    parent_tree.node_collection("SN_PieMenuNode"),
                    "refs",
                    text="",
                    icon="VIEWZOOM",
                    item_search_property="name",
            ...
            if self.parent_type == "BLENDER":
                ...
                subrow.prop_search(
                    self,
                    "ref_SN_OperatorNode",
                    parent_tree.node_collection("SN_OperatorNode"),
                    "refs",
                    text="",
                    icon="VIEWZOOM",
                    item_search_property="name",
