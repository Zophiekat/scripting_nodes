...
class ScriptingSocket:
    ...
    def _get_value(self):
        ...
        attr_name = self.subtype_attr
        storage_key = self._get_socket_storage_key(f"_{attr_name}")
        value = self.node.get(storage_key, self.default_prop_value)
        try:
            if isinstance(self.default_prop_value, int) and not isinstance(self.default_prop_value, bool):
                return int(value)
            elif isinstance(self.default_prop_value, float):
                return float(value)
            elif isinstance(self.default_prop_value, bool):
                return bool(value)
            elif isinstance(self.default_prop_value, str):
                return str(value)
            return value
        except Exception:
